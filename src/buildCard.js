...
export function buildCardHTML({ games, story, injuries, standingsMover, top10, startDate, endDate }) {
  ...
  const upsets     = top10
    ? games.filter(g => g.away.won && top10.has(g.home.abbr)).length
    : 0;
  ...
  const gameStatsHTML = buildInfoCard({
    ...
    custom: `
      ...
      <div class="stat-row-mini"><span class="srl">Upset</span><span class="srv" style="color:var(--gold)">${pct(upsets)}</span><span class="src">${upsets} games</span></div>
...
function getBadge(game, top10 = new Set()) {
  ...
  if (game.away.won && top10.has(game.home.abbr)) return { cls: 'badge-upset', text: 'Upset' };
  return null;
}
